...
mbspls_flip_weights.list <- function(x, signs = -1L, flip_boot = TRUE, flip_T = TRUE, ...) {
    ...
    if (length(signs) == 1L && !is.matrix(signs)) {
      ...
      if (!is.null(rownames(S0)) && !is.null(colnames(S0))) {
        ...
        if (nrow(S0) == K && ncol(S0) == B) {
          ...
          S0 <- t(S0)
        } else stop("signs matrix must be KxB or BxK")
  ...
  .fix_bounds <- function(lo, hi) {
    lo2 <- pmin(lo, hi, na.rm = TRUE); hi2 <- pmax(lo, hi, na.rm = TRUE)
    list(lo2, hi2)
  }
  ...
  if (isTRUE(flip_boot)) {
    ...
    if (!is.null(x$weights_stability_filtered)) {
      for (k in seq_len(K)) for (b in seq_len(B)) {
        s <- S[k, b]; if (is.na(s) || s == 1L) next
        blk <- bn[b]
    ...
    if (!is.null(x$weights_ci)) {
      ...
      comp_col <- "component"; block_col <- "block"
        ...
        if (any(sel)) {
          ...
          if (all(c("ci_lower","ci_upper") %in% names(df))) {
            ...
            if (length(ii)) {
              ...
              df$ci_lower[ii] <- z[[1]]; df$ci_upper[ii] <- z[[2]]
            }
          }
          if (all(c("ci_lower_nz","ci_upper_nz") %in% names(df))) {
            ...
            if (length(ii)) {
              ...
              df$ci_lower_nz[ii] <- z[[1]]; df$ci_upper_nz[ii] <- z[[2]]
            }
          }
        }
      }
