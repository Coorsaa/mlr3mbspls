...
# Define site/batch correction variables **by block**
# (each entry lists site/batch and optional covariates present in the task backend)
site_correction <- list(
  clinical = c("site", "batch"),
  genes = c("site", "batch"),
  metabolites = c("site", "batch")
)

# Choose a correction method per block: "partial_corr" (default), "combat", or "dir"
site_correction_methods <- list(
  clinical = "partial_corr",
  genes = "partial_corr",
  metabolites = "partial_corr"
)
...
# Keep site/batch columns available for `PipeOpSiteCorrection`
# (they will be dropped automatically after correction unless `keep_site_col = TRUE`).
...
viz_graph <- po("scale") %>>%
  po("mbspls", 
     ...
     performance_metric = "mac",
     permutation_test = TRUE,
     val_test = "bootstrap",  # Prediction-side bootstrap validation (used by autoplot on predict())
     val_test_n = 100L,  # Reduced for demo
     val_test_alpha = 0.05,
...
# This requires val_test = "bootstrap" and calling predict() at least once
tryCatch({
  p_bootstrap <- autoplot(viz_learner, type = "mbspls_bootstrap_component")
  print(p_bootstrap)
}, error = function(e) {
  cat("Bootstrap plot not available - requires val_test = 'bootstrap' and calling predict()\n")
})
```
...
cat("   - Use po('blockscale', method = 'unit_ssq') for equal block importance\n")
...
complete_graph <- 
  ...
  po("blockscale", blocks = blocks, method = "unit_ssq") %>>%  # Block scaling
  po("mbspls", 
     blocks = blocks,
     ncomp = 3L,
     performance_metric = "mac",
...
## Bootstrap Stability Selection (Parallel via `future`)

`PipeOpMBsPLSBootstrapSelect` can compute stability-filtered weights and stable latent variables using bootstrap resampling.
To parallelize the bootstrap loop **cross-platform**, use the `future` ecosystem.

```{r}
# Optional: parallelize bootstrap stability selection (cross-platform)
...
stable_graph = po("blockscale", blocks = blocks, method = "unit_ssq") %>>%
  po("mbspls",
     blocks = blocks,
     ncomp = 3L,
     performance_metric = "mac",
     # store the original blocks for the bootstrap selector
     store_train_blocks = TRUE,
     # keep non-block features (if any) so the selector can cleanly replace upstream LV columns
     append = TRUE,
     log_env = log_env) %>>%
...
stable_learner = as_learner(stable_graph)
stable_learner$train(task)

# After training, stability outputs are available in the selector state and/or log_env
stable_learner$model$mbspls_bootstrap_select$kept_blocks_per_comp
```
